...
class LSTMRNN(object):
    '''
        input_size:输入的维度
        output_size：输出的维度
        本例子中input_size,output_size=1,因为每次input的时候都是一个值y,每一个批次也只有一个output
        这里假定cell_size=10
    '''
    ...
    def compute_cost(self):
        ...
        with tf.name_scope('average_cost'):
            self.cost = tf.div(
                ...
                tf.cast(self.batch_size,tf.float320),
                name = 'average_cost'
            )
...
if __name__ == '__main__':
    ...
    for i in range(200):
        ...
        _,cost,state,pred = sess.run(
            [model.train_op,model.cost,model.cell_fianl_state,model.pred],
            feed_dict=feed_dict)
