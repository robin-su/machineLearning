...
def mle():  # 0B/1M/2E/3S
    ...
    for k, token in enumerate(tokens):
        ...
        if n == 1:  # 等于1表示single
            ...
            b[3][ord(token[0])] += 1 # 从3到token[0]又多了一次
            last_q = 3 # 将当前状态设置成3
            continue
        # 初始向量
        '''
        在：一个字就能成为词语，用Single表示
        给：给表示一个词的起始状态，用Begin表示
        定：表示一个词的终止状态，用end表示
        在“中华人民共和国”中，中表示Begin
        国表示end,华人民共和表示Middle
        '''
        pi[0] += 1  # 只要长度不是0，肯定一定多了一个begin和end  # begin +1
        pi[2] += 1  # end +1
        pi[1] += (n-2) # 如果存在中间Middle，那么他肯定等于总的个数减去2
        # 转移矩阵
        a[last_q][0] += 1
        last_q = 2
        if n == 2:
            a[0][2] += 1  # 从begin直接转到end+1
        else:
            a[0][1] += 1
            a[1][1] += (n-3)
            a[1][2] += 1
