...
if __name__ == "__main__":
    ...
    # fit_predict相当于两个动作的合并：km_cluster.fit（data）+km_cluster.predict（data），可以一次性得到聚类预测之后的标签，免去了中间过程。
    y_hat = cls.fit_predict(data)
    y2_hat = cls.fit_predict(data2)
    y3_hat = cls.fit_predict(data3)

    m = np.array(((1,1),(1,3)))
    data_r = data.dot(m) # 计算矩阵的内积
    ...
    plt.subplot(421) # 分成4 * 2,占用第一个，即第一行第一列的子图
    plt.title(u'旋转后的数据')
    plt.scatter(data[:,0],data[:,1],c=y, s=30,cmap=cm,edgecolors='none')
    x1_min,x2_min = np.min(data,axis=0) # 每列的最小值
    x1_max,x2_max = np.max(data,axis=0) # 每行的最小值
