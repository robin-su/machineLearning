...
if __name__ == "__main__":
    data = np.loadtxt(r'D:\machineLearning\src\zhouboML\SVM\data\14.bipartition.txt', dtype=np.float, delimiter='\t')
    x, y = np.split(data, (2,), axis=1)
    y[y == 0] = -1  # 将y==0的都转换成-1
    y = y.ravel()  # ravel转换成行向量
    ...
    for i, clf in enumerate(clfs):  # 枚举分类器
        clf.fit(x, y)
        ...
        # levels 指定距离是1，0，还是-1.linewidths 决定每个距离对应的图中线段的粗细
